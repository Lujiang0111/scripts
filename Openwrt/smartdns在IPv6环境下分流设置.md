...
2. 配置主DNS服务器
    ...
    + 点击**高级设置**选项卡，按下表所示配置。
        ...
        | 停用IPv6地址解析 | ❌ |
        | 停用Https地址解析 | ❌ |
    + 如果需要劫持dnsmasq，则需要勾选```自动设置dnsmasq```，保存设置。点击luci界面左侧**网络**->**DHCP/DNS**，检查**DNS 转发**项是否被设置为```127.0.0.1#5333```。
...
+ OpenClash设置页面，点击**覆写设置**，设置自定义上游DNS服务器
  ...
  + 选择**NameServer**选项卡，添加并**只保留**如下服务器

    | 启用 | 服务器分组 | 服务器地址 | 服务器端口 | 服务器类型 |
    | - | - | - | - | - |
    | ✔ | NameServer | 127.0.0.1 | 5333 | UDP |

  + 选择**FallBack**选项卡，添加并**只保留**如下服务器

    | 启用 | 服务器分组 | 服务器地址 | 服务器端口 | 服务器类型 |
    | - | - | - | - | - |
    | ✔ | FallBack | 127.0.0.1 | 5335 | UDP |
